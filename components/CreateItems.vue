    ...
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        <div class="flex items-center">
          <DialogTitle class="w-full">Criar Item </DialogTitle>
        ...
        <DialogDescription class="flex justify-start ">
          <button @click="navigatePage('edit_items')" class="uppercase bg-[var(--orange)] active:scale-95 flex space-x-2 border p-1 rounded-md">
            <div>
              editar
            </div>
...
const createItems = async () =>{
    ...
    await useMetallurgy().showAllMelorryId(useModal().idCategory)
