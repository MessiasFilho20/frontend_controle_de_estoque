<template>
   <div class="flex w-full space-x-3 px-2 my-1">
    <div v-if="use_users.user.role === 'admin'">
      <button @click="ClickmenuAdmin()" class="px-2 active:scale-95 flex justify-center border bg-[var(--orange)] rounded-sm">
      ...
      <div class="flex w-full">
        <input class="w-full bg-[var(--darkorange)]  outline-none pl-1 rounded-l-sm" type="text"> 
        <button class="px-2 active:scale-95 flex justify-center items-center border bg-[var(--orange)] rounded-r-sm">
...
const ClickmenuAdmin = () =>{
    useModal().menuadmin = true
}
